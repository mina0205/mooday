...
export default function InvitePage() {
  ...
  useEffect(() => {
    const loadUserAndCouple = async () => {
      ...
      const { data } = await supabase.from('couple_members').select('couple_id').eq('user_id', session.user.id).maybeSingle();
      if (data?.couple_id) setHasCouple(true);
      setLoading(false);
    };
    loadUserAndCouple();
  }, []);
    ...
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={handleCopyInviteCode} activeOpacity={0.7}>
        <View style={styles.codeBox}>
          <Text style={styles.codeText}>{inviteCode ?? '아직 초대 코드가 없어요'}</Text>
        </View>
      </TouchableOpacity>

      <TouchableOpacity
        style={[styles.primaryButton, (loading || hasCouple) && { opacity: 0.5 }]}
        onPress={handleCreateInviteCode}
        disabled={loading || hasCouple}
      >
        <Text style={styles.primaryText}>
          {hasCouple ? '이미 커플이 연결되어 있어요' : loading ? '생성 중...' : '초대 코드 생성하기'}
        </Text>
      </TouchableOpacity>

      <View style={styles.divider} />

      <TouchableOpacity style={styles.secondaryButton} onPress={() => router.push('/invite/input')}>
        <Text style={styles.secondaryText}>초대 코드 입력하기 →</Text>
      </TouchableOpacity>
    </View>
  );
}
